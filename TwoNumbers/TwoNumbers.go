...
// a target number. It should find two different items in the array that, when added together, 
...
var i, j int 
func twoNumbers(slice []int, target int) [2]int {
	i = 0
	for ; i < len(slice); i++{
		j = i + 1
		for ;j < len(slice); j++ {
			if (target == (slice[i] + slice[j])) {
				return [2]int{i, j}
			}
		}
	}
