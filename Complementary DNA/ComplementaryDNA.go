...
// as "C" and "G". You have function with one side of the DNA (string, except for Haskell); you need to get the other complementary side. DNA strand is never empty or there is no DNA at all (again, except for Haskell).
...
func dnaStrand(dna string) string {
	...
	arr := strings.Split(dna, "")
	var returned []string
	for _, el := range arr {
		returned = append(returned, voc[el])
	}
	return strings.Join(returned, "")
}
